...
// POST /api/pedidos — cliente finaliza o carrinho
router.post('/', authCliente, async (req, res) => {
  const { itens, forma_pagamento, cupom_codigo, endereco, frete, dados_pagamento } = req.body;
  ...
  try {
    ...
    if (cupom_codigo) {
      ...
      if (cupons.length > 0) {
        ...
        desconto = cupons[0].desconto_percent;
      }
    }
    ...
    const freteValor    = frete?.valor ? Number(frete.valor) : 0;
    const total_final   = total - valorDesconto + freteValor;

    // Pagamentos via MP ficam como 'pago'; demais ficam 'pendente'
    const statusInicial = ['mercadopago', 'cartao'].includes(forma_pagamento) ? 'pago' : 'pendente';
    ...
    const [pedidoResult] = await conn.query(
      `INSERT INTO pedidos
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        req.cliente.id, total, valorDesconto, total_final,
        forma_pagamento, statusInicial, cupom_id,
        ...
        frete?.servico || null, freteValor, frete?.prazo || null,
      ]
    );
    ...
    for (const item of itens) {
      const [produto] = await conn.query(
        'SELECT preco, nome FROM produtos WHERE id = ?',
        [item.produto_id]
      );
      await conn.query(
    ...
    // Busca dados completos do cliente para o e-mail
...
// PUT /api/pedidos/:id/status — atualiza status (admin)
